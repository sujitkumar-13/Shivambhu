  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-slate-800">Customer Feedback</h1>
          <p className="text-sm lg:text-base text-slate-500 mt-1 font-medium">Manage testimonials shown on the home page</p>
        </div>
        <button
          onClick={() => {
            setEditingTestimonial(null)
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        {loading && testimonials.length === 0 ? (
          ...
          testimonials.map((t) => (
            <div key={t.id} className="bg-white p-6 lg:p-8 rounded-[1.5rem] lg:rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-100/50 flex flex-col justify-between group lg:hover:-translate-y-2 transition-transform duration-300">
              <div>
                <div className="flex items-center justify-between mb-4 lg:mb-6">
                  <div className="flex items-center gap-0.5 lg:gap-1">
                    {[1, 2, 3, 4, 5].map(s => (
                      <i key={s} className={`ri-star-fill ${s <= t.rating ? 'text-yellow-400' : 'text-slate-200'} text-xs lg:text-sm`}></i>
                    ))}
                  </div>
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => handleEdit(t)}
                      className="p-2 text-slate-400 lg:text-slate-300 hover:text-cyan-600 transition-colors"
                      title="Edit"
                    >
                    ...
                    <button
                      ...
                      className="p-2 text-slate-400 lg:text-slate-300 hover:text-red-500 transition-colors"
                      title="Delete"
                    >
                ...
                <p className="text-slate-600 text-sm leading-relaxed mb-6 lg:mb-8 italic font-medium">"{t.content}"</p>
              </div>
              <div className="flex items-center gap-4 pt-6 border-t border-slate-50">
                <div className="w-10 h-10 lg:w-12 lg:h-12 bg-slate-100 rounded-full overflow-hidden flex items-center justify-center text-slate-400 uppercase font-bold border border-slate-200 flex-shrink-0">
                  {t.image ? (
        ...
        {showModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center overflow-y-auto py-10 px-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-white rounded-[1.5rem] lg:rounded-[2.5rem] shadow-2xl p-6 lg:p-10 z-[70] max-h-[90vh] overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
            >
